...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask):
        ...
        hs, attens = self.decoder(tgt, memory, memory_key_padding_mask=mask, tgt_key_padding_mask=tgt_mask,
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        attens = []
        for layer in self.layers:
            output, atten = layer(output, memory, tgt_mask=tgt_mask,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory,
        ...
        # i.e., input of location 1: <start>, output of location 1: 'a' (may not)
        # attn_mask: 用來遮擋正確答案
        # key_padding_mask: 每一個句子長度不一樣, 但格式固定下需要將沒有使用到的資訊mask
        tgt2, sentence_attn = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask,
        ...
        # attn_output_weights = torch.matmul(sentence_attn, attn_output_weights)
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
