...
def train_epoch(source_dataloader, target_dataloader, epoch, total_epoch):
    ...
    for i, ((source_data, source_label), (target_data, target_label)) in enumerate(zip(source_dataloader, target_dataloader)):
        ...
        # target training labels for validation to choose the best model
...
if __name__=='__main__':
    same_seeds(1123)
    source_dataset = get_dataset(os.path.join('hw2_data', 'digits', 'mnistm'), 'train')
    target_dataset = get_dataset(os.path.join('hw2_data', 'digits', 'usps'), 'train')
    ...
    for epoch in range(n_epoch):
        ...
        if best_acc < target_acc:
            print('save!')
            best_acc = target_acc
            torch.save(feature_extractor.state_dict(), f'extractor_model_m2u.pth')
            torch.save(label_predictor.state_dict(), f'predictor_model_m2u.pth')
